...
struct list_node : public list_node_base<T> {
  ...
  explicit list_node(const T& v) : value(v) {}

  explicit list_node(T&& v) : value(ccystl::move(v)) {}
...
struct list_iterator
    : public ccystl::iterator<ccystl::bidirectional_iterator_tag, T> {
...
class list {
 ...
 public:
  ...
  template <class Iter,
            typename std::enable_if<ccystl::is_input_iterator<Iter>::value,
                                    int>::type = 0>
 ...
 public:
  ...
  void unique() { unique(ccystl::equal_to<T>()); }

  template <class BinaryPredicate>
  void unique(BinaryPredicate pred);

  void merge(list& x) { merge(x, ccystl::less<T>()); }

  template <class Compare>
  void merge(list& x, Compare comp);

  void sort() { list_sort(begin(), end(), size(), ccystl::less<T>()); }
...
typename list<T>::node_ptr list<T>::create_node(Args&&... args) {
  ...
  try {
    data_allocator::construct(ccystl::address_of(p->value),
...
template <class T>
bool operator<(const list<T>& lhs, const list<T>& rhs) {
  return ccystl::lexicographical_compare(lhs.cbegin(), lhs.cend(), rhs.cbegin(),
                                         rhs.cend());
}
