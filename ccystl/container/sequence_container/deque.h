...
class deque {
 ...
 public:
  ...
  template <class IIter,
            typename std::enable_if<ccystl::is_input_iterator<IIter>::value,
                                    int>::type = 0>
...
void deque<T>::map_init(size_type nElem) {
  ...
  map_size_ =
      ccystl::max(static_cast<size_type>(DEQUE_MAP_INIT_SIZE), nNode + 2);
  try {
    ...
    try {
      if (elems_before >= n) {
        ...
        ccystl::uninitialized_fill(
            ccystl::uninitialized_copy(begin_, position, new_begin), begin_,
            value_copy);
        begin_ = new_begin;
...
void deque<T>::reallocate_map_at_front(size_type need_buffer) {
  const size_type new_map_size = ccystl::max(
