...
struct char_traits<char16_t> {
  ...
  static size_t length(const char_type* str) noexcept {
    size_t len = 0;
    for (; *str != static_cast<char_type>(0); ++str) ++len;
    return len;
  }
...
class basic_string {
 public:
  ...
  static_assert(std::is_pod_v<CharType>,
 ...
 public:
  ...
  template <class Iter,
            std::enable_if_t<ccystl::is_input_iterator<Iter>::value, int> = 0>
...
void basic_string<CharType, CharTraits>::fill_init(size_type n, value_type ch) {
  const auto init_size =
      ccystl::max(static_cast<size_type>(STRING_INIT_SIZE), n + 1);
...
void basic_string<CharType, CharTraits>::copy_init(
    Iter first, Iter last, ccystl::forward_iterator_tag) {
...
void basic_string<CharType, CharTraits>::init_from(const_pointer src,
                                                   ...
                                                   size_type count) {
  const auto init_size =
      ccystl::max(static_cast<size_type>(STRING_INIT_SIZE), count + 1);
  buffer_ = data_allocator::allocate(init_size);
