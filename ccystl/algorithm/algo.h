...
#include "ccystl/algorithm/range_algo.h"
...
/*****************************************************************************************/
/**
 * @brief 检查[first, last)内是否全部元素都满足一元操作 unary_pred 为 true
 * 的情况
 *
 ...
 * @param first 区间起始迭代器
 * @param last 区间结束迭代器
 * @param unary_pred 一元操作
 * @return bool 如果全部元素都满足条件返回 true，否则返回 false
 */
/*****************************************************************************************/
...
/*****************************************************************************************/
/**
 * @brief 检查[first, last)内是否存在某个元素满足一元操作 unary_pred 为 true
 * 的情况
 *
 * @tparam InputIter 输入迭代器类型
 * @tparam UnaryPredicate 一元谓词类型
 ...
 * @param last 区间结束迭代器
 * @param unary_pred 一元操作
 * @return bool 如果存在元素满足条件返回 true，否则返回 false
 */
/*****************************************************************************************/
template <class InputIter, class UnaryPredicate>
...
/*****************************************************************************************/
/**
 * @brief 检查[first, last)内是否全部元素都不满足一元操作 unary_pred 为 true
 * 的情况
 *
 * @tparam InputIter 输入迭代器类型
 * @tparam UnaryPredicate 一元谓词类型
 ...
 * @param last 区间结束迭代器
 * @param unary_pred 一元操作
 * @return bool 如果全部元素都不满足条件返回 true，否则返回 false
 */
/*****************************************************************************************/
template <class InputIter, class UnaryPredicate>
...
/*****************************************************************************************/
/**
 * @brief 对[first, last)区间内的元素与给定值进行比较，返回元素相等的个数
 *
 * @tparam InputIter 输入迭代器类型
 * @tparam T 要比较的值的类型
 * @param first 区间起始迭代器
...
/*****************************************************************************************/
/**
 * @brief 对[first, last)区间内的每个元素都进行一元 unary_pred 操作，返回结果为
 * true 的个数
 *
 * @tparam InputIter 输入迭代器类型
 * @tparam UnaryPredicate 一元谓词类型
 ...
 * @param last 区间结束迭代器
 * @param value 要查找的值
 * @return InputIter 指向找到元素的迭代器，若未找到则返回 last
 */
/*****************************************************************************************/
template <class InputIter, class T>
InputIter find(InputIter first, InputIter last, const T& value) {
  // 遍历区间内元素，直到找到目标值或到达区间末尾
...
/*****************************************************************************************/
/**
 * @brief 在[first, last)区间内找到第一个令一元操作 unary_pred 为 true
 * 的元素并返回指向该元素的迭代器
 *
 * @tparam InputIter 输入迭代器类型
 * @tparam UnaryPredicate 一元谓词类型
...
/*****************************************************************************************/
/**
 * @brief 在[first, last)区间内找到第一个令一元操作 unary_pred 为 false
 * 的元素并返回指向该元素的迭代器
 *
 * @tparam InputIter 输入迭代器类型
 * @tparam UnaryPredicate 一元谓词类型
 ...
 * @param last 区间结束迭代器
 * @param unary_pred 一元操作
 * @return InputIter 指向不满足条件元素的迭代器，若未找到则返回 last
 */
/*****************************************************************************************/
template <class InputIter, class UnaryPredicate>
InputIter find_if_not(InputIter first, InputIter last,
                      UnaryPredicate unary_pred) {
  // 遍历区间内元素，直到找到不满足条件的元素或到达区间末尾
  while (first != last && unary_pred(*first)) ++first;
  return first;
}
 ...
 *
 * @tparam ForwardIter1 第一个区间的前向迭代器类型
 * @tparam ForwardIter2 第二个区间的前向迭代器类型
 * @param first1 第一个区间的起始迭代器
 * @param last1 第一个区间的结束迭代器
 * @param first2 第二个区间的起始迭代器
 * @param last2 第二个区间的结束迭代器
 * @return ForwardIter1 指向首次出现点的迭代器，若未找到则返回 last1
 */
/*****************************************************************************************/
...
ForwardIter1 search(ForwardIter1 first1, ForwardIter1 last1,
  ...
  if (first2 == last2)
    return first1;  // 如果第二个区间为空，直接返回第一个区间的起始位置
  while (first1 != last1) {
    ...
    if (it2 == last2) {
      return first1;  // 找到第二个区间，返回起始位置
    }
    ++first1;
  }
  return last1;  // 未找到第二个区间，返回第一个区间的结束位置
}
...
template <class ForwardIter, class T>
ccystl::pair<ForwardIter, ForwardIter> erange_dispatch(ForwardIter first,
                                                       ForwardIter last,
                                                       const T& value,
...
ccystl::pair<RandomIter, RandomIter> erange_dispatch(RandomIter first,
                                                     RandomIter last,
                                                     const T& value,
                                                     random_access_iterator_tag,
                                                     Compared comp) {
...
void reverse_dispatch(RandomIter first, RandomIter last,
                      random_access_iterator_tag) {
  while (first < last) ccystl::iter_swap(first++, --last);
}
...
BidirectionalIter rotate_dispatch(BidirectionalIter first,
  ...
  while (first != middle && middle != last) ccystl::swap(*first++, *--last);
  if (first == middle) {
    ccystl::reverse_dispatch(middle, last, bidirectional_iterator_tag());
    return last;
  } else {
...
void merge_without_buffer(BidirectionalIter first, BidirectionalIter middle,
  ...
  if (len1 + len2 == 2) {
    if (*middle < *first) ccystl::iter_swap(first, middle);
    return;
  }
...
BidirectionalIter1 merge_backward(BidirectionalIter1 first1,
  ...
  if (first1 == last1) return ccystl::copy_backward(first2, last2, result);
  if (first2 == last2) return ccystl::copy_backward(first1, last1, result);
  --last1;
  --last2;
  while (true) {
    if (*last2 < *last1) {
...
void merge_without_buffer(BidirectionalIter first, BidirectionalIter middle,
  ...
  if (len1 == 0 || len2 == 0) return;
  if (len1 + len2 == 2) {
    if (comp(*middle, *first)) ccystl::iter_swap(first, middle);
    return;
  }
  auto first_cut = first;
  auto second_cut = middle;
...
ccystl::pair<OutputIter1, OutputIter2> partition_copy(
...
OutputIter unique_copy(InputIter first, InputIter last, OutputIter result) {
  ...
  return ccystl::unique_copy_dispatch(first, last, result,
