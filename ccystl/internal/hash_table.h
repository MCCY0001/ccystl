...
struct hashtable_node {
  ...
  hashtable_node(hashtable_node&& node) noexcept
      : next(node.next), value(ccystl::move(node.value)) {
    node.next = nullptr;
  }
};
...
template <class T>
struct ht_local_iterator
    : public ccystl::iterator<ccystl::forward_iterator_tag, T> {
  typedef T value_type;
  typedef value_type* pointer;
  typedef value_type& reference;
  typedef size_t size_type;
  ...
  explicit ht_local_iterator(node_ptr n) : node(n) {}
...
struct ht_const_local_iterator
  ...
  explicit ht_const_local_iterator(node_ptr n) : node(n) {}
...
class hashtable {
 ...
 public:
  ...
  hashtable(Iter first, Iter last, size_type bucket_count,
      ...
      : size_(ccystl::distance(first, last)),
        mlf_(1.0f),
        hash_(hash),
        equal_(equal) {
    init(ccystl::max(bucket_count,
  ...
  static size_type max_size() noexcept { return static_cast<size_type>(-1); }
  ...
  [[nodiscard]] float max_load_factor() const noexcept { return mlf_; }
  ...
  void reserve(size_type count) {
    rehash(static_cast<size_type>(
        static_cast<float>(count) / max_load_factor() + 0.5f));
  }
...
hashtable<T, Hash, KeyEqual>::emplace_multi(Args&&... args) {
  ...
  try {
    if (static_cast<float>(size_ + 1) >
        static_cast<float>(bucket_size_) * max_load_factor())
      rehash(size_ + 1);
  } catch (...) {
    destroy_node(np);
    throw;
...
void hashtable<T, Hash, KeyEqual>::rehash(size_type count) {
  ...
  if (n > bucket_size_) {
    ...
    if (static_cast<float>(size_) / static_cast<float>(n) <
            max_load_factor() - 0.25f &&
        static_cast<float>(n) <
            static_cast<float>(bucket_size_) * 0.75)  // worth rehash
    {
      replace_bucket(n);
    }
  }
    ...
    if (is_equal(value_traits::get_key(first->value), key)) {
      ...
      for (node_ptr second = first->next; second; second = second->next) {
        if (!is_equal(value_traits::get_key(second->value), key))
          return ccystl::make_pair(iterator(first, this),
...
hashtable<T, Hash, KeyEqual>::create_node(Args&&... args) {
  ...
  try {
    data_allocator::construct(ccystl::address_of(tmp->value),
...
// destroy_node 函数
template <class T, class Hash, class KeyEqual>
void hashtable<T, Hash, KeyEqual>::destroy_node(node_ptr n) {
...
void hashtable<T, Hash, KeyEqual>::copy_insert_multi(
    InputIter first, InputIter last, ccystl::input_iterator_tag) {
...
template <class ForwardIter>
void hashtable<T, Hash, KeyEqual>::copy_insert_multi(
    ForwardIter first, ForwardIter last, ccystl::forward_iterator_tag) {
...
bool hashtable<T, Hash, KeyEqual>::equal_to_multi(const hashtable& other) {
  ...
  for (auto f = begin(), l = end(); f != l;) {
    ...
    if (ccystl::distance(p1.first, p1.last) !=
            ccystl::distance(p2.first, p2.last) ||
