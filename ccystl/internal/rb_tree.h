...
class rb_tree {
 ...
 public:
  ...
  ccystl::pair<iterator, iterator> equal_range_multi(const key_type& key) {
    return ccystl::pair<iterator, iterator>(lower_bound(key), upper_bound(key));
  }

  ccystl::pair<const_iterator, const_iterator> equal_range_multi(
      const key_type& key) const {
    return ccystl::pair<const_iterator, const_iterator>(lower_bound(key),
                                                        upper_bound(key));
  }

  ccystl::pair<iterator, iterator> equal_range_unique(const key_type& key) {
    iterator it = find(key);
    auto next = it;
    return it == end() ? ccystl::make_pair(it, it)
                       : ccystl::make_pair(it, ++next);
  }

  ccystl::pair<const_iterator, const_iterator> equal_range_unique(
      const key_type& key) const {
 ...
 private:
  ...
  // get insert pos
  ccystl::pair<base_ptr, bool> get_insert_multi_pos(const key_type& key);
  ccystl::pair<ccystl::pair<base_ptr, bool>, bool> get_insert_unique_pos(
      const key_type& key);
...
rb_tree<T, Compare>::get_insert_unique_pos(const key_type& key) {
  ...
  // 表示是否在左边插入， 第二个值为一个 bool，表示是否插入成功
  auto x = root();
  auto y = header_;
